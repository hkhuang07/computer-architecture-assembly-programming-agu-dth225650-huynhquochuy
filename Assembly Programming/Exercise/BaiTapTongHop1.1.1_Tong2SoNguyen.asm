.data
    ...
    msg_sum:    .asciiz "Tong hai so nguyen la: "   # Chuỗi thông báo kết quả tổng
...
main:
    ...
    # -----------------------------------------------------
    # 4. Tính tổng hai số nguyên
    # -----------------------------------------------------
    add $s0, $t0, $t1     # add: Cộng giá trị của thanh ghi $t0 (số thứ nhất)
    ...
    # -----------------------------------------------------
    la $a0, msg_sum       # load address: Nạp địa chỉ của chuỗi 'msg_sum' vào thanh ghi $a0.
    li $v0, 4             # load immediate: Mã dịch vụ 4 là 'print_string'.
    syscall               # Thực hiện syscall để in chuỗi thông báo "Tong hai so nguyen la: ".
